...
export default function MobileProjectCard({ project, onClick }) {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className="flex items-center w-full h-[72px] bg-neutral-900 border border-neutral-800 overflow-hidden cursor-pointer group text-left active:scale-[0.98] transition-transform"
    >
      {/* Thumbnail */}
      <div className="w-[90px] h-full shrink-0 overflow-hidden">
        <img
          src={project.img}
          alt={project.title}
          loading="lazy"
      ...
      <div className="flex-1 px-3 flex flex-col justify-center gap-1 min-w-0">
        <span className="font-mono text-[8px] text-orange-500 uppercase tracking-[0.2em] truncate">
          {project.type}
        </span>
        <span className="text-[15px] font-black text-white uppercase tracking-tighter leading-none truncate">
          {project.title}
        </span>
